...
Either `use_cookie=False` (it is on by default, so check nothing turned it off),
or `cookie_secure=True` while you are browsing over plain `http://`. A Secure
cookie is silently discarded by the browser on a non-HTTPS origin — no error,
it simply never appears. Use `cookie_secure=False` in development.
</Fix>
